...
## 134. Gas Station
给出数组gas[]表示在位置i能加到的油,数组cost表示位置i到i+1所需要的油,
车子油箱无限大小,求能顺时针走完一圈的出发点,数据保证答案唯一或不存在.

解法一: 模拟法, 遍历数组两遍, 求**子数组的最大和**, 顺便记录其起始点,
模拟从起始点出发看能否完成一圈

解法二: 可以证明: 若总油量大于总耗油,则一定存在合法的出发点,
由于题目数据保证只有一个合法出发点,每当油量为负,则重置出发点为下一个点,
这样可保证得到一个合法出发点
```java
class Solution {
...
## 135. Candy
给出一个数组代表小朋友的分数, 给出一个最小分糖方案,
保证每个小朋友最少一个糖,且分数比旁边邻居高的小朋友得到更多糖

解法: 初始化一个均为1的数组, 
从左往右扫, 若当前位置分数比左边邻居大,则糖果为左边邻居+1,
