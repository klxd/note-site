...
### 基本概念
* 集群角色: Leader(所有机器通过选举过程选定), Follower, Observer(不参与选举,不参与写过半则成功策略)
* 会话Session:默认端口2181
* 数据节点Znode:
 持久节点: 一旦创建, 除非主动移除, 否则一直保存
 临时节点: 生命周期和客户端会话绑定, 会话失效则自动移除
 顺序属性: 节点可以标记上Sequential属性, 那么节点在创建时Zookeeper会自动在后面追加一个整形自增数字
* 版本: Znode中的Stat
* Watcher: 事件监听器, 允许用户注册watcher, 订阅感兴趣的事件
* ACL: access control list权限控制
...
2. watcher event异步发送watcher的通知事件从server发送到client是异步的，这就存在一个问题，不同的客户端和服务器之间通过socket进行通信，
   由于网络延迟或其他因素导 致客户端在不通的时刻监听到事件，由于Zookeeper本身提供了ordering guarantee，即客户端监听事件后，才会感知它所监视znode发生了变化。
   所以我们使用Zookeeper不能期望能够监控到节点每次的变化。Zookeeper只能保证最终的一致性，而无法保证强一致性。
3. 数据监视Zookeeper有数据监视和子数据监视getdata() and exists()设置数据监视，getchildren()设置了子节点监视。
4. 注册watcher: getData、exists、getChildren
5. 触发watcher: create、delete、setData
6. setData()会触发znode上设置的data watch（如果set成功的话）。
   一个成功的create() 操作会触发被创建的znode上的数据watch，以及其父节点上的child watch。而一个成功的delete()操作将会同时触发一个znode的data watch
   和child watch（因为这样就没有子节点了），同时也会触发其父节点的child watch。
...
## 数据发布/订阅
将配置信息存放到ZK上进行集中管理, 应用启动时主动到ZK服务端进行一次配置信息的获取, 同时在指定节点上注册一个Watcher监听,
一旦信息发生变更, ZK会通知所有的订阅的客户端

## 负载均衡
动态DNS服务

## 命名服务
全局唯一ID -- 分布式环境中, 上层应用仅仅需要一个全局唯一的名字, 类似数据库中的主键(分库分表时数据库无法提供)
ZK实现: 创建顺序节点, 调用创建接口会返回节点的完整名字, 如job-0000000002, 此名字即全局唯一ID

## 分布式协调/通知
分布式系统机器间通信方式
* 心跳检测, 客户端在同一个节点下创建临时子节点, 不同的机器间通过这个临时节点是否存在来判断对应的机器是否存活
* 工作进度汇报, 客户端创建临时子节点, 实时将自己的任务执行进度写到这个节点上去
* 系统调度, 分布式系统由控制台和客户端两部分组成, 后台管理人员的做一些操作, 实际上就是修改ZK上的节点数据, ZK则把变更通知给客户端

## 集群管理
分布式日志收集系统, 解决变化的日志源机器和变化的收集器机器两个问题, 收集器创建临时节点, 日志源自己选择负载较小的收集器节点,
往上面再创建临时子节点

## Master选举
关系型数据库的主键特性来实现, 所有机器往数据库中插入一条相同主键id的记录, 只有一台能成功, 这样能进行选举, 
但是master挂了的时候无法通知到其他节点, 即无法动态选举.
ZK实现选举, 客户端每天定时往ZK上创建一个临时节点, 如`master/2019-12-12/binging`, 这个过程中, 只有一个客户端能成功, 它就成为Master,
其他节点则往`master/2019-12-12`上注册一个节点变更的watcher, 用于监控当前master是否存活, 如果master挂了, 其余客户端可以重新进行选举

## 分布式队列
* FIFO先进先出队列, 往同一个节点上创建临时顺序子节点, 获取该节点下所有子节点的信息, 若自己已经是最小节点, 则出队, 否则往比自己序号小的那个节点注册监听watcher
* 分布式屏障Barrier: 节点`/queue_barrier`是一个已经存在的默认节点, 节点中数据为10, 表示当其子节点的数量达到10之后, 才会打开Barrier.
  客户端往该节点下注册临时子节点, 并获取子节点信息, 若达到10则出队, 否则监听子节点列表信息变化, 进入等待 
...
A13:Leader 崩溃之后，集群会选出新的Leader，然后就会进入恢复阶段，新的Leader具有所有已经提交的提议，因此它会保证让followers同步已提交的提议，丢弃未提交的提议（以 Leader 的记录为准），这就保证了整个集群的数据一致性。
