...
class Game2048 extends Component {
  ...
  static setNewNum = arr => {
    ...
    if (position >= 0) {
      arr[Math.floor(position / 4)][
        position % 4
      ] = Game2048.generateNewCellNumber()
    }
    return position
  }
          ...
          {this.state.arr.map((row, rowIndex) => (
            <div key={rowIndex} className="grid-row">
              {row.map((num, columnIndex) => {
                let newClass =
                  rowIndex * 4 + columnIndex === this.state.newCellPosition
                    ? "tile-new "
                    : ""
                ...
                return (
                  <div
                    key={columnIndex}
                    className={"grid-cell " + newClass + numClass}
                  >
                    {num === 0 ? "" : num}
                  </div>
                )
